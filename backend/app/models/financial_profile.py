...
class FinancialProfile(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    money_mix_type: Mapped[str] = mapped_column(String(20), default="home", nullable=False)
    receives_salary_besides_business: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    business_reserve_amount: Mapped[Optional[float]] = mapped_column(nullable=True)
    daily_needs_override: Mapped[Optional[float]] = mapped_column(nullable=True)
